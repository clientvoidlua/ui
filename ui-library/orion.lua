...
--Lucide Icons https://github.com/Footagesus/Icons/blob/main/lucide/dist/Icons.lua
local Icons = {}

local Success, Response = pcall(function()
	local Raw = game:HttpGet("https://raw.githubusercontent.com/Footagesus/Icons/main/lucide/dist/Icons.lua")

	Raw = Raw:gsub("%[\"(.-)\"%]%s*=%s*\"(.-)\"", '"%1":"%2"')
	Raw = "{" .. Raw:match("{(.*)}") .. "}"
...
if not Success then
	warn("\nOrion Library - Failed to load Lucide Icons. Error code: " .. tostring(Response) .. "\n")
end
...
local function GetIcon(Icon)
	...
	warn("Icon not found: " .. tostring(Icon))
	return nil
end
...
CreateElement("Image", function(ImageID)
	local FinalImage = ImageID

	if GetIcon(ImageID) ~= nil then
		FinalImage = GetIcon(ImageID)
	end
...
function OrionLib:MakeNotification(NotificationConfig)
	spawn(function()
		...
		local FinalImage = GetIcon(NotificationConfig.Image) or NotificationConfig.Image
		...
		local NotificationFrame = SetChildren(
			...
			),
			{
				...
				SetProps(MakeElement("Image", FinalImage), {
...
function OrionLib:MakeWindow(WindowConfig)
	...
	WindowConfig.ToggleKey = WindowConfig.ToggleKey or Enum.KeyCode.K
	...
	WindowConfig.Icon = GetIcon(WindowConfig.Icon) or WindowConfig.Icon
	WindowConfig.IntroIcon = GetIcon(WindowConfig.IntroIcon) or WindowConfig.IntroIcon
	...
	AddConnection(CloseBtn.MouseButton1Up, function()
		...
		OrionLib:MakeNotification({
			...
			Content = "Press "..WindowConfig.ToggleKey.Name.." to reopen the interface",
			Time = 5
		})
	...
	AddConnection(MinimizeBtn.MouseButton1Up, function()
		if Minimized then
			...
			MinimizeBtn.Ico.Image = GetIcon("square") or "rbxassetid://7072720870"
...
function TabFunction:MakeTab(TabConfig)
	...
	local FinalIcon = GetIcon(TabConfig.Icon) or TabConfig.Icon

	local TabFrame = SetChildren(SetProps(MakeElement("Button"), {
		Size = UDim2.new(1, 0, 0, 30),
		Parent = TabHolder
	}), {
		AddThemeObject(SetProps(MakeElement("Image", FinalIcon), {
...
UserInputService.InputChanged:Connect(function(Input)
	...
	or input.UserInputType == Enum.UserInputType.Touch then
		...
		ColorInput = AddConnection(RunService.RenderStepped, function()
			...
			local ColorX = (math.clamp(Pos.X - Color.AbsolutePosition.X, 0, Color.AbsoluteSize.X) / Color.AbsoluteSize.X)
			local ColorY = (math.clamp(Pos.Y - Color.AbsolutePosition.Y, 0, Color.AbsoluteSize.Y) / Color.AbsoluteSize.Y)
...
AddConnection(Hue.InputBegan, function(input)
	...
	or input.UserInputType == Enum.UserInputType.Touch then
		...
		HueInput = AddConnection(RunService.RenderStepped, function()
			...
			local HueY = (math.clamp(Pos.Y - Hue.AbsolutePosition.Y, 0, Hue.AbsoluteSize.Y) / Hue.AbsoluteSize.Y)
